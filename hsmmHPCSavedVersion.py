...
class HSMM():
   def __init__(self, pnn, couple_type,n_resampling,alpha,gamma,mu, lmda, kappa,nu,k,theta,n_states,n_trunc):
      ...
      self.file_prefix = 'results/%s_%s_%s_%s_%s_%s_%s' %\
   ...
   def simulate_individual(self,index):
      ...
      #print 'Simulated data for couple %s in Group: %s; Affect by Sex' % (index,self.couple_type)
      sim_affect = shelve.open('~/results/simCoupleAffect.db')      
      for i in range(len(sub_state_seq_X)):
         print  '%g,%g' % (sub_state_seq_X[i],sub_state_seq_Y[i])     
         sim_affect[str(index)+'_'+str(i)] = sub_state_seq_X[i],sub_state_seq_Y[i]
      sim_affect.close() 
