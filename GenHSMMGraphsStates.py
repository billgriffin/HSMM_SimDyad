...
if len(sys.argv) < 4:
    ...
    exit
    '''python GenHSMMGraphStates.py conf/paramterX.conf results/correspond_result.txt couple_index

   Ex: generate the result graph of first couple:
   python GenHSMMGraphsStates.py conf/parameter11.conf results5/Summary/All_Cluster1_11_10000_4.0_8.0_20_60.txt 2
   '''
...
for idx in range(num_couples):
    ...
    for state in range(number_of_states):
        if state in stateseq_norep:
            ...
            colors = [cmap(sum(i)/9) for i in projected_data]
    ...
    plt.title('Male-Female Joint Affect Distribution')
    ...
    print States
    plt.plot(States, color = "#348ABD", lw=1.8)
    plt.title('Simulated Affect States Trace')
    ...
    #for i in range(len(realdata)):
        #realStates.append((realdata[:,0] + realdata[:,1])-1) 
    print realStates
    plt.plot((realdata[:,0] + realdata[:,1])-1, color="#348ABD", lw=1.8)    
    plt.title('Original Affect States Trace')
