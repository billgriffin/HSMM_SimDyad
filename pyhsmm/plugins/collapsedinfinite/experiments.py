...
def generate_hsmm_data():
    return pyhsmm.models.HSMM(
            [pyhsmm.basic.distributions.ScalarGaussianNIX(**obs_hypparams) for s in range(10)],
            [pyhsmm.basic.distributions.PoissonDuration(**dur_psn_hypparams) for s in range(10)],
            alpha=6.,gamma=6.,init_state_concentration=6.).generate(50)

def generate_hmm_data():
    return pyhsmm.models.HSMM(
            [pyhsmm.basic.distributions.ScalarGaussianNIX(**obs_hypparams) for s in range(10)],
            [pyhsmm.basic.distributions.GeometricDuration(**dur_geo_hypparams) for s in range(10)],
            alpha=6.,gamma=6.).generate(50)
