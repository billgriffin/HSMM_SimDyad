...
'''
python GenHSMMGraphs.py conf/parameter24.conf results1/Summary/All_Cluster1_24_10000_8.0_8.0_20_90.txt 4

python GenHSMMGraphs.py conf/parameter72.conf results1/Summary/All_Cluster3_72_10000_8.0_8.0_20_90.txt 1

python GenHSMMGraphs.py conf/parameter120.conf results1/Summary/All_Cluster5_120_10000_8.0_8.0_20_90.txt 5
'''
...
while len(line) > 0:
    ...
    if startSystem and re.match(r'^[0-9]', line.strip()):  # fix: 0-9
        ...
        state_colors[state] = (D_1_Avg + D_2_Avg )/10 #need more discrimination
...
startMu = False
while len(line) > 0:
    if not startMu and line.strip().startswith("Simulated States for couple indexed as"):
        startMu= True
        line = f.readline()
...
for idx in range(num_couples):
    ...
    plt.imshow(z, cmap="RdBu_r", extent=[0,4,0,4])
    x = [0,1,2,3,4]
    ...
    for state in range(number_of_states):
        if state in stateseq_norep:
            ...
            colors = [cmap(sum(i)/10) for i in projected_data]
            ...
            plot_gaussian_projection(mu_dict[state],sigmaState,vecs,color=obs_color,label='')
    ...
    plt.plot(sub_state_seq_X,  color='b' )
    plt.plot(sub_state_seq_Y,  color='r')
    ...
    #plt.plot(realdata[:,0], label ='Male', color='b' )
    #plt.plot(realdata[:,1], label = 'Female', color='r')
    plt.plot(realdata[:,0],  color='b' )
    plt.plot(realdata[:,1],  color='r')
